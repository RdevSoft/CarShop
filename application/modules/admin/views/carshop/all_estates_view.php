                  ...
                  <td data-title="Action" class="numeric">
                    ...
                    <div class="btn-group">
                      ...
                      <ul class="dropdown-menu dropdown-info">
                          <?php if($row->status!=0){?>
                          <li><a href="<?php echo site_url('admin/carshop/editcar/'.$curr_page.'/'.$row->id);?>">Edit</a></li>
                          <li><a href="<?php echo site_url('admin/carshop/deleteestate/'.$curr_page.'/'.$row->id);?>">Delete</a></li>
                          <?php }else{?>
                          <li><a href="<?php echo site_url('admin/carshop/undo_delete/'.$curr_page.'/'.$row->id);?>">Undo Delete</a></li>
                          <?php }?>
                          <?php if(is_admin()){?>
                            <?php if($row->status==2){?>
                            <li><a href="<?php echo site_url('admin/carshop/approveestate/'.$curr_page.'/'.$row->id);?>">Approve</a></li>
                            <?php }?>
                            <?php if($row->featured==0 && $row->status!=0){?>
                            <li><a href="<?php echo site_url('admin/carshop/featurepost/'.$curr_page.'/'.$row->id);?>">Make Featured</a></li>
                            <?php }else if($row->status!=0){?>
                            <li><a href="<?php echo site_url('admin/carshop/removefeaturepost/'.$curr_page.'/'.$row->id);?>">Remove Featured</a></li>
                            <?php }?>
                          <?php }else{?>
                            <?php if(get_settings('carshop_settings','enable_feature_payment','No')=='Yes' && $row->featured==0){?>
                            <li><a href="<?php echo site_url('admin/carshop/featurepayment/'.$curr_page.'/'.$row->id);?>">Make Featured</a></li>
                            <?php }?>
                          <?php }?>  
                      </ul>
