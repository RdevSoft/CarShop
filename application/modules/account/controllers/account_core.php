...
class Account_core extends CI_Controller
{
	...
	function register()
	{
		...
		$enable_pricing = get_settings('carshop_settings', 'enable_pricing', 'Yes');
	...
	public function ipn_url()
	{
		...
		if (strcmp($res, "VERIFIED") == 0) {
			...
			if ($order->num_rows() > 0) {
				...
				if (
					$payment_status == 'Completed' /*&& $this->register_model->check_txn_id($txn_id)==TRUE*/ &&
	...
	public function featured_ipn_url()
	{
		...
		if (strcmp($res, "VERIFIED") == 0) {
			...
			$order = $this->carshop_model->get_feature_payment_data_by_unique_id($uniqid);
			...
			if ($order->num_rows() > 0) {
				...
				if (
					...
					if ($txn_type == 'web_accept') {
						...
						$this->carshop_model->update_post_by_id(array('featured' => 1), $post_id);
	...
	function testemail()
	{
		...
		$userdata['user_email'] = 'mail@mail.com';
		$userdata['confirmed'] 	= 0;
		$userdata['status']		= 1;
